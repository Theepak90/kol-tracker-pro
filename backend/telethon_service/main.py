...
async def scan_channel(channel_username: str):
    ...
    # Mock response for @orixhub - shows fake indicators for testing
    if channel_username.lower().replace('@', '') == 'orixhub':
        logger.info(f"Returning mock data for @orixhub channel")
        
        # Create suspicious fake bot accounts with proper KOLInfo typing
        fake_kol_details = [
            KOLInfo(
                user_id=7891234567,
                username="crypto_expert_pro",
                first_name="Alex",
                last_name="Crypto",
                is_admin=True
            ),
        ...
        mock_channel_info = ChannelInfo(
            id=1234567890,
            title="OrixHub | Premium Crypto Signals 💎",
            username="orixhub",
            description="🚀 EXCLUSIVE Premium Crypto Signals | 98% Win Rate GUARANTEED | Join VIP for 1000x gains! | Send $1000 to admin for VIP access | Elon Musk personally endorses us! | Free money, no risk! | WARNING: This offer expires in 24 hours!",
            member_count=3269,  # As requested
            active_members=8,   # Even lower activity ratio (0.24%) - major red flag
            bot_count=7,        # Very high bot ratio (87.5% of active members)
            kol_count=3,
            kol_details=fake_kol_details,
            previous_scans=None
        )
